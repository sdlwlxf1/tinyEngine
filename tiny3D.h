...
// 绘制扫描线
void device_draw_scanline(device_t *device, scanline_t *scanline) {
    IUINT32 *framebuffer = device->framebuffer[scanline->y]; // 获取当前扫描线所在对应的帧缓存
    float *zbuffer = device->zbuffer[scanline->y]; // 获取当前扫描线所对应的z缓存
    int x = scanline->x; // 扫描线的起点x
    int w = scanline->w; // 扫描线的宽度w
    int width = device->width; // 设备的宽度
    int render_state = device->render_state; // 设备的状态
    for (; w > 0; x++, w--) {
        if (x >= 0 && x < width) {
            float rhw = scanline->v.rhw;
            if (rhw >= zbuffer[x]) {
